...
const ItemSidebar = ({ activeTab, setActiveTab }) => {
  ...
  const menuItems = [
    { id: 'dashboard', title: 'Dashboard', icon: 'dashboard' },
    {
      ...
      icon: 'inventory_2',
      subItems: [
        { id: 'all_items', title: 'All Inventory' },
        { id: 'ppe_kits', title: 'PPE Kits' },
        { id: 'diagnostics', title: 'Diagnostics' },
        { id: 'supplies', title: 'Medical Supplies' },
      ]
    },
    { id: 'requests', title: 'Requests', icon: 'rebase_edit' },
    { id: 'needs', title: 'Item Campaigns', icon: 'campaign' },
  ...
  return (
    <aside className="fixed left-0 top-0 h-full flex flex-col z-40 w-72 bg-[#0a1526] border-r border-white/5 overflow-hidden">
      ...
      <div className="px-8 py-10 relative z-10">
        <div className="flex items-center gap-3">
          ...
          <div className="min-w-0">
            ...
            <div className="text-[10px] font-semibold uppercase tracking-[0.2em] text-slate-500 mt-1 whitespace-nowrap">Stewardship Panel</div>
          </div>
        </div>
      </div>
      ...
      <nav className="flex-1 px-4 py-2 space-y-1 relative z-10 overflow-y-auto font-['Work_Sans']">
        <div className="text-[10px] font-bold uppercase tracking-widest text-slate-600 px-4 mb-4">Main Navigation</div>
        {menuItems.map((item) => (
          <motion.button
            key={item.id}
        ...
        <div className="flex items-center gap-4 mb-6 px-2">
          <div className="relative">
            <div className="w-12 h-12 rounded-2xl bg-gradient-to-tr from-slate-700 to-slate-800 border border-white/10 flex items-center justify-center overflow-hidden">
              <span className="text-lg font-bold text-white uppercase">{user?.username?.charAt(0) || 'I'}</span>
            </div>
            <div className="absolute -bottom-1 -right-1 w-4 h-4 bg-green-500 border-2 border-[#0a1526] rounded-full"></div>
          </div>
          <div className="flex-1 min-w-0">
            <div className="text-sm font-semibold text-white truncate">{user?.username || 'Item Manager'}</div>
            <div className="text-[11px] font-medium text-slate-500 truncate lowercase">Inventory Lead</div>
          </div>
        </div>
