...
const HospitalBloodRequests = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      const res = await api.get(`/hospital-requests/blood/hospital/${hId}`);
  ...
  const handleDelete = async (requestId) => {
    if (!window.confirm("Are you sure you want to withdraw this hematology coordination request?")) return;
    try {
      await api.delete(`/hospital-requests/blood/${requestId}`);
      fetchData();
    } catch (err) {
      alert(err.response?.data?.message || "Failed to withdraw blood request");
    }
  };
  ...
  return (
    <div className="p-10 space-y-10 max-w-[1700px] mx-auto animate-in fade-in slide-in-from-bottom-4 duration-700 font-['Work_Sans']">
      <section className="flex justify-between items-end">
        <div>
          <h1 className="text-5xl font-extrabold tracking-tight text-slate-900">Blood Requests</h1>
          <p className="text-slate-500 mt-3 font-medium text-lg">Request life-saving pints from the central blood repository.</p>
        </div>
        <button
          disabled={!hospitalId}
          onClick={() => {
          ...
          className="px-8 py-4 bg-red-600 text-white font-bold rounded-2xl shadow-xl shadow-red-600/20 hover:scale-[1.05] active:scale-95 transition-all flex items-center gap-3 disabled:opacity-50 disabled:hover:scale-100"
        >
